...
= 1.5.7 =
* UI: rij "Update-server" verborgen op de instellingenpagina (interne backend-URL hoeft niet zichtbaar te zijn voor klanten)
...
* Verbetering: standalone admin preview simuleert nu zowel oneven als even theme-bleed (twee verschillende grijstinten), zodat dit type regressie meteen zichtbaar is in de preview vóór een live deploy.

= 1.5.4 =
* Wijziging: nieuwe kolommen en cellen worden standaard gecentreerd uitgelijnd
* Uitzondering: kolom 1 van een nieuwe tabel blijft standaard links uitgelijnd
* Bestaande tabellen blijven ongewijzigd; alle uitlijningen zijn nog steeds handmatig aanpasbaar via de toolbar

= 1.5.3 =
* Structurele fix: alle visueel relevante CSS-selectoren voor de plugin-tabel zijn gequalified als `.tmp-wrapper .tmp-table ...` zodat WordPress theme-styling (padding, font, achtergrond, alignment) niet meer doorbloed naar TableMaster-tabellen op de live website
* Verbetering: standalone admin preview simuleert nu ook theme-bleed van padding, achtergrond, font-family en text-align (niet alleen borders) — als een tabel hier visueel correct rendert, dan ook op live
