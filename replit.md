...
- Auto-update: plugin checks API server's `/api/wp-update/info` endpoint; update URL hardcoded (not user-editable) via `TMP_UPDATE_URL` constant or fallback default. SHA-256 hash verification on downloaded packages. No blocking `sleep()` — uses transient-based error caching (30 min backoff) instead.
